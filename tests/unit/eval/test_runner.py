...
def _job(
    ...
    *,
    ...
    candidate = EvaluationJob(
        ...
        metric=metric,
        artifact_kind=artifact_kind,
        prompt_manifest_path="evaluation/prompts.json",
...
def test_runner_publishes_metrics_manual_index_and_complete_tree(tmp_path: Path) -> None:
    ...
    assert (
        result.output_path / "inputs" / "prompt-manifest.json"
...
def test_preflight_reports_each_missing_external_identity_as_a_blocker(
    ...
    config = cast(
        RuntimeConfig,
        SimpleNamespace(
            run=SimpleNamespace(intent="eval"),
            distributed=SimpleNamespace(world_size=1, backend="native"),
            paths=SimpleNamespace(artifact_dir="artifacts/eval"),
                evaluation=SimpleNamespace(
                    explicit_job=True,
                    gpu_index=0,
                    prompt_manifest_path="evaluation/missing-prompts.json",
                prompt_manifest_sha256=_HASH_A,
                sampling=SimpleNamespace(profile="reference"),
                fid=SimpleNamespace(
                    feature_extractor_path="evaluation/missing-extractor.pt",
                    feature_extractor_sha256=_HASH_B,
                    preprocess_path="evaluation/missing-preprocess.pt",
                    preprocess_sha256=_HASH_C,
                    real_stats_path="evaluation/missing-real-stats.safetensors",
                    real_stats_sha256=_HASH_D,
                ),
            ),
        ),
    ...
    assert blockers["PROMPT_MANIFEST_IDENTITY_INVALID"].endswith(
        "evaluation/missing-prompts.json"
    )
    assert blockers["FEATURE_EXTRACTOR_IDENTITY_INVALID"].endswith(
        "evaluation/missing-extractor.pt"
    )
    assert blockers["PREPROCESS_IDENTITY_INVALID"].endswith(
        "evaluation/missing-preprocess.pt"
    )
    assert blockers["REAL_STATS_IDENTITY_INVALID"].endswith(
        "evaluation/missing-real-stats.safetensors"
    )
...
def test_injected_execution_requires_permanent_engineering_classification(
    ...
    plan = _plan(tmp_path, include_is=False, include_manual=False)
...
def _preflight_config(prompts: PromptManifest) -> RuntimeConfig:
    evaluation = SimpleNamespace(
        ...
        manual_quality=SimpleNamespace(enabled=False, samples=4),
    )
    return cast(
        RuntimeConfig,
        SimpleNamespace(
...
def _patch_preflight_dependencies(
    ...
    *,
    ...
    def fake_verify(path: Path, expected: str) -> VerifiedLocalFile:
        return VerifiedLocalFile(path, expected, 1)
    ...
    def fake_prompts(_path: Path) -> PromptManifest:
        return prompts
    ...
    monkeypatch.setattr(runner_module, "load_prompt_manifest", fake_prompts)
...
def test_raw_strict_jlt_provenance_comes_from_resolved_checkpoint_config(
    ...
    growth = SimpleNamespace(
        ...
        world_size=1,
        resolution=256,
        active_slot_ids=tuple(range(16)),
        alpha=1.0,
    )
...
def test_stage_end_requires_exact_checkpoint_set_and_never_creates_publisher(
    ...
    assert "STAGE_END_PROMPT_CONDITION_CONTRACT_UNRESOLVED" in codes

    forged = dataclasses.replace(_plan(tmp_path), stage_end=True)
...
def test_complete_stage_end_chain_reaches_only_the_unresolved_condition_gate(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    _patch_preflight_dependencies(monkeypatch, _PROMPTS, update=10)
    ...
    def fake_raw(
        ...
        return runner_module._RawProvenance(
            expected, "S0", 1, 256, tuple(range(16)), 1.0
        )
...
def test_preflight_blocks_ungoverned_nonempty_prompt_conditions(
    ...
    assert "PROMPT_CONDITION_CONTRACT_UNRESOLVED" in {
        item.code for item in captured.value.blockers
    }

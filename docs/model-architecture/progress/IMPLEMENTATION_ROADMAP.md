...
- **动作：** JLT `P_mean=-0.8/P_std=0.8`、noise_scale1、t_eps0.05；`z_t=t*x+(1-t)*epsilon`；网络x-pred；FP32 x-to-v；per-sample mean后global mean；velocity CFG2.9；Heun-50+final Euler/99 NFE。
- **验证：** t=0 noise/t=1 clean；禁止clean叫z0；cond/uncond分别x-to-v后CFG；solver state FP32；固定ODE golden和NFE计数；DDP global mean留到 `T041`。

...
- `DatabaseAsset.required_for_runtime=true` is rejected by the strict audit-only schema and both unit and contract negative tests.
- The capability types are slotted and weak-referenceable under the locked Python 3.12 runtime. Exact instances have no `__dict__`, so `object.__setattr__` cannot shadow any of the five public consumer methods. Fingerprints directly read slots through `object.__getattribute__`; they do not dispatch through an instance method.
- Tests exercise `VerifiedAssetFile.require_unchanged`/`verified_root` and `VerifiedAssetSelection.require_unchanged`/`verified_path`/`verified_root` through both instance and class calls. Every consumer rejects field mutation and runtime/database grafting; every same-name method injection raises before the forged callable can be installed or invoked.
- Database runtime scope now uses a strict bool plus an allow-only-false field validator. TOML `true`, integers `0/1`, and strings fail parsing, while direct strict model validation rejects `None`; no bool/int coercion remains.
- Test evidence records the 12 production modules present in the latest isolated `f7ceb222`-based commit candidate.
...
- Fourth-review A001-only isolate (`f7ceb2229874986492f807863b33f803ebec9566` plus five A001 code/test files): 115 targeted tests passed in 16.92 seconds and 260 full-suite tests passed in 21.77 seconds.
- The same isolate passed full Ruff, strict Pyright with 0 errors/0 warnings, traceability with 221 requirements/221 source nodes/16 archive files/12 production modules/235 runtime keys, and the current stricter A002 scanner with 28 sources/0 violations.

The latest clean candidate was cloned at base `f7ceb2229874986492f807863b33f803ebec9566` and received only the explicit A001 fourth-review diff. No D001 checker/test diff, D010 data/config diff, or A002 scanner diff entered it; the current scanner was used externally for a read-only compatibility scan. No model payload, database row, `.env`, GPU, or performance placeholder was read or created during this remediation. The affected implementation remains on the existing `C02-001`/`C03-001` paths, so registry revision 11 is unchanged; both requirements remain below `verified`, and T020/T021 retain the real model loading, posterior/forward, round-trip, and 1GPU gates.

...
- Capability narrowing accepts only exact `VerifiedAssetSelection` and nested `VerifiedAssetFile` instances issued by the successful preflight factories. Separate identity-keyed weak registries use `weakref() is object`, never dataclass equality; locks protect issue/check/GC cleanup without retaining capabilities. Direct constructors, `object.__new__`, equal-field copies, and nested subclass overrides therefore cannot mint or replace a live capability. This A001-owned guard closes the capability-forgery prerequisite identified during A002 review.
...
Sealed-capability remediation and isolated CPU verification are complete. The `80692a7`-based A001-only candidate passes 55 targeted tests, 200 full-suite tests, full Ruff, strict Pyright, and traceability; its GC/thread-safety test also passes 10 consecutive runs. Declared runtime models are fully locked through one readiness contract; databases and development references are explicit audit-only scopes. This work remains static/hash readiness evidence only and does not close T020/T021 real-load gates. Independent Foundation AI/Infra re-review and main-agent acceptance remain pending.

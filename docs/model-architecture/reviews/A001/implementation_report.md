...
- `DatabaseAsset.required_for_runtime` is strictly `Literal[False]`. The current audit-only database decision is therefore enforced during manifest parsing instead of relying on the runtime inspector to ignore an unsupported `true` value.

...
## Eighth remediation: post-`cadd873` independent review

- Function positional defaults, keyword-only defaults, and dynamic annotation expressions now reject every security-capability fact at definition time. This covers verified model roots, sensitive model loaders, issued asset selections, and live D010 target/header/connection facts. Exact frozen D010 network class type annotations are separately audited as static types, not live capabilities.
- Callable annotation/default namespaces are denied through `__annotations__`, `annotation`, `return_annotation`, `typing.get_type_hints`, direct attributes, and literal `getattr`. The same rule covers the synthetic Git fixture, so `make_reference` cannot be stored in a default and later recovered to bypass its safe-relative argument check.
- `functools.partial` and `partialmethod` may not bind reflection callables such as `getattr`, `vars`, or `__getattribute__`; they also may not bind the synthetic Git helper. Dynamic builtins/module recovery through `sys.modules` or `__builtins__`, and bulk reflection through `inspect.getmembers`/`getmembers_static`, fail closed.
- The A002 contract now carries a compressed, payload-hashed UTF-8 fixture containing the complete frozen `ModelScopeDatasetTransport` class, a minimal module prelude, and inert helper summaries. It is only decompressed and parsed/scanned, never imported or executed. Its normalized class digest must equal the scanner pin and the full fixture must scan with zero violations.
- When the shared D010 source exists, a second contract requires its complete normalized class AST to equal the tracked fixture. In a clean A002-only checkout that test is skipped, while the tracked fixture/pin success-path test still runs. This removes the circular dependency on an uncommitted D010 source.

## Eighth-remediation clean validation and remaining gates

- The detached candidate used base `cadd87391ef07ed7edb2932e8e2b339b268838ff` and overlaid only `tools/asset_execution_boundary.py` and `tests/contracts/assets/test_asset_execution_boundary.py`; concurrent D001/D010 and all evidence edits were excluded.
- Results: 307 passed / 1 skipped boundary contracts in 25.22 seconds pytest-reported time (25.838 wall), 505 passed / 1 skipped full tests in 47.18 seconds (47.784 wall), 28 Python sources with zero violations in 20.249 seconds, full Ruff pass in 0.273 seconds, strict Pyright with zero errors/warnings in 3.991 seconds, and traceability verification in 1.054 seconds for 221 requirements/source nodes, 16 archives, 12 production modules, and 235 runtime config keys. The only skip is the optional shared-D010 equality check; the tracked fixture digest and zero-violation pin path passed.
- In the shared tree, the D010 equality contract ran and passed: 308/308 boundary contracts, plus 36 Python sources with zero violations in 31.595 seconds. This is static compatibility evidence only; D010 owns remote streaming and file-safety evidence.
- No `.env`, model, database, dataset/cache, or `reference/` payload was read. No network access, model/data download, GPU work, or performance artifact was performed. Status remains pending independent AI and Infra rereview and main-agent acceptance.

## Ninth remediation: reflection callable classification and module namespaces

- A single canonical callable-name helper strips any number of trailing `.__call__` segments. Direct special handling now uses that canonical name, so calling `getattr` through one or repeated `.__call__` segments retains the same selected-member fact instead of falling through as an ordinary call.
- Reflection selectors and namespace inspectors form one security-sensitive callable class. The existing recursive fact and higher-order escape checks now reject this class through `partial`/`partialmethod`, `operator.call`, `map`/`next`, local wrappers and unknown cross-module adapters; no adapter-specific allowlist was added.
- Production `.__dict__` access is classified as namespace reflection and returns a conservative security fact after reporting. This blocks chained recovery from `sys`, `builtins`, `importlib`, Transformers and other module/class namespaces.
- New negative contracts reproduce every independent Infra bypass and retain a positive literal local-member `getattr.__call__` case. The existing loader/root/asset-selection, synthetic Git, D010 network capability, source-TOCTOU, fixed-point and frozen-fixture matrices run unchanged as adjacency coverage.

## Ninth-remediation clean validation and remaining gates

- The isolated source candidate used exact base `477d0a76027dec67dd42c8824108238415d6728f` and overlaid only the A002 tool and contract paths; D001, D010 and evidence changes were excluded.

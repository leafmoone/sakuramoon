...
- 每个 successful optimizer update 至少记录 高噪区域和低噪区域的loss以及总loss、pre-clip grad norm、post-clip grad norm、clip fraction、learning rate、timestep 分布、有效 batch、image/text tokens、samples/s、显存和 non-finite 计数。

...
# core/vision_encoder/pe.py, reduced to the vision-encoder classes required
# by the SakuraMoon iREPA frozen teacher (LayerScale, AttentionPooling,
# SelfAttention, ResidualAttentionBlock, Transformer, VisionTransformer).
# The text tower, CLIP wrapper, and checkpoint fetcher are intentionally not
# vendored.  Class bodies are copied verbatim except that import paths were
# re-based to the local pe_ref package (this VBS audit's self-contained copy
# of the iprea-branch vendored package), the timm DropPath import was re-based
# to the locally vendored .drop_path (timm is not installed in the audit venv;
# drop_path is 0.0 in PE-Spatial-B16-512 so the module is never active),
# unused imports were trimmed, VisionTransformer.from_config's pretrained path
# now requires a local checkpoint_path (the huggingface_hub fetcher is not
# vendored), and the SelfAttention SDPA call is wrapped in a MATH-backend
# context (the DTK/ROCm torch build registers the flash SDPA backend but ships
# without its kernel library, so auto-selection crashes instead of falling
# back; see _make_sdpa_backend_context).
# Apache-2.0; see LICENSE.PE.

"""Official PE vision encoder (PE-Spatial compatible), frozen for iREPA."""
...
from .config import PE_VISION_CONFIG
...
def _make_sdpa_backend_context():
    """Pin the MATH SDPA backend for this vendored attention path.

    The DTK/ROCm torch build registers the flash SDPA backend but ships
    without its kernel library (flash_attn_2_cuda*.so); backend
    auto-selection then crashes with "No matching libraries found" instead
    of falling back, and neither the memory-efficient nor the cuDNN
    attention kernels are compiled in.  The MATH backend is always
    available and bitwise-deterministic, which the frozen-teacher parity
    and determinism tests rely on.  Falls back to a no-op context on torch
    versions without the SDPA backend selector (the probe then uses
    whatever backend the process default picks).
    """
...
_SDPA_BACKEND = _make_sdpa_backend_context()
        ...
        **kwdargs,
        ...
        if pretrained:
            if checkpoint_path is None:
                raise RuntimeError(
                    "pretrained loading requires a local checkpoint_path; "
                    "the Hugging Face fetcher is intentionally not vendored"
                )

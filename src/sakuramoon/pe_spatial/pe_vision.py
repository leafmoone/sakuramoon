...
# re-based to sakuramoon.pe_spatial, unused imports were trimmed,
# VisionTransformer.from_config's pretrained path now requires a local
# checkpoint_path (the huggingface_hub fetcher is not vendored), and the
# SelfAttention SDPA call is wrapped in a MATH-backend context (the DTK/ROCm
# torch build registers the flash SDPA backend but ships without its kernel
# library, so auto-selection crashes instead of falling back; see
# _make_sdpa_backend_context).

...
per-sample mean velocity loss, and the x-to-v-then-CFG ordering.
"""
...
def _require_open_unit_float(name: str, value: object) -> float:
    value = _require_finite_float(name, value)
...
def _x_prediction_to_velocity(
    ...
    _require_open_unit_float("t_eps", t_eps)
...
def sample_jlt_timesteps(
    ...
    *,
    ...
    _require_positive_float("p_std", p_std)
    normal = torch.randn(
        batch_size,
        device=device,
        dtype=torch.float32,
...
def sample_noise(
    ...
    *,
    ...
    _require_positive_float("noise_scale", noise_scale)
    return (
        torch.randn(
            clean.shape,
            device=clean.device,
...
def flow_matching_loss(
    ...
    *,
    ...
    _require_open_unit_float("t_eps", t_eps)
    _require_open_unit_float("noise_observation_boundary", noise_observation_boundary)
    _validate_batch_timestep(timestep, state, validate_range=True)
...
def guided_velocity(
    ...
    *,
    ...
    _require_open_unit_float("t_eps", t_eps)
    _require_finite_float("guidance_scale", guidance_scale)

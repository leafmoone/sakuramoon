...
Vendored from Microsoft Mage-Flow (MIT), file
``mage_flow/models/modules/mage_vae.py`` of the microsoft/Mage repository
(https://github.com/microsoft/Mage), vendored for the anime-sr U-Flow
project so that the SR codebase stays independent of the text-to-image
runtime (plan §8: only the pretrained Mage-VAE is reused, everything
else is reimplemented).

Local modifications vs upstream:
  * ``loguru`` replaced by the standard-library ``logging`` (no new deps).

Contract (plan §4):
  * ``encode([B, 3, H, W])`` in ``[-1, 1]`` → ``[B, 128, H/16, W/16]``,
    deterministic posterior mean when ``sample_posterior=False``.
  * ``decode([B, 128, h, w])`` → ``[B, 3, 16h, 16w]`` in ``[-1, 1]``
    (single-step t=0 denoiser decode).
...
logger = logging.getLogger("anime_sr.vae.mage_vae")
...
class NerfEmbedder(nn.Module):
    ...
    @lru_cache  # noqa: B019  # upstream pattern (per-instance cache keyed on (patch_size, device, dtype))
    ...
    def _load_decoder(self, sd, ckpt_path):
        ...
        for k, v in sd.items():
            if not k.startswith(prefix):
                continue
            new_k = k[len(prefix):]
            if new_k.startswith(("y_embedder.encoder.", "y_embedder.bottleneck.")):
                continue
